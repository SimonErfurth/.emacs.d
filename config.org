...
is made up of code stolen from configurations found all around, and a (small)
amount of code that I have written myself. The =init.el=, as well as most of the
original base of this file is stolen from *Uncle Dave's Emacs*.
** Dependencies
For flyspell we need a language, like ~aspell-en~ (me think). For All the icons
we need to run ~all-the-icons-install-fonts~ after downloading the package.
~pdftk~ for being able to rotate pdf-files. or python we need ~pip install jedi
rope isort falke8 autopep8~.
** Notes that should not need to be included
According to Uncle Dave the following is dependencies for programming in this Emacs.
*** Programming
**** Package manager
...
** a prerequisite or two for others packages
And a pre-prerequisite for this
#+BEGIN_SRC emacs-lisp
  (use-package ivy
    :ensure t
    :init
...
#+BEGIN_SRC emacs-lisp
  (use-package yasnippet
    :ensure t
    :config
    (setq yas-snippet-dirs (append yas-snippet-dirs '("/home/ss-blackthorn/ss-config/emacs/snippets")))
    (yas-reload-all))
