...
* MacOS <2024-09-05 Thu>
Generally, MacOS is similar enough to Linux, that we can mostly just use the
Linux dependencies listed above. Instead of using the hyper modifier, which
deosn't exist on MacOS, we rebind everything to =C-M-s=, and rebind capslock to
that using hyperkey.app.

And don't forget to unbind C-Spc from changing input source in setting.
...
#+BEGIN_SRC emacs-lisp
  (use-package js2-refactor
    ...
    :config (js2r-add-keybindings-with-prefix "H-r"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (eval-after-load 'tex-mode
    '(define-key LaTeX-mode-map  (kbd "H-<tab>") 'indent-for-tab-command))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package pdf-tools
    ...
    ;; Disabled - I also use pdf-tools on MacOS, and tbh who uses Windows.
...
#+begin_src emacs-lisp
  (use-package mu4e
    ...
    :hook (mu4e-main-mode . (lambda () (load-theme 'doom-ayu-dark t)))
    :config
