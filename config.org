...
- From your package manager
  ...
  - texlab :: We have to manually install a LSP-server for LaTeX https://github.com/latex-lsp/texlab
...
Additionally, see email section for separate setup.

** Mac
Generally mac is similar enough to Linux for most of the above to be relevant.
Instead of using the hyper modifier (which isn't supported on macos), we use
simlar enough to, and bind caps-lock to C-M-s, and change all H- prefixes to
this.
* Diminish
...
spacemacs slow, however it would be stupid not to acknowledge the best parts
about it; their modified powerline setup.
...
#+BEGIN_SRC emacs-lisp
  (use-package helm
    ...
    ;; Custom function inspired by OP: https://www.reddit.com/r/emacs/comments/x0r0pe/share_your_otherwindow_commands/
    (defun sse-switch-buffer-other-window ()
      "In other window switch buffer using helm-buffers-list"
      (interactive)
    ...
    (global-set-key (kbd "C-x B") 'sse-switch-buffer-other-window))
...
Dired is the build in file navigator in Emacs, and it is pretty awesome, but its
age might show. Here we give it some cosmetic life-assistance and create a hydra
for easier discover-ability.
...
#+begin_src emacs-lisp
  (use-package dirvish
    ...
    :config
    (global-set-key (kbd "M-m d") 'dirvish-dwim)
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-M-s-i") 'previous-line)
  (global-set-key (kbd "C-M-s-k") 'next-line)
  (global-set-key (kbd "C-M-s-l") 'forward-char)
  (global-set-key (kbd "C-M-s-j") 'backward-char)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package avy
    ...
    (("M-s" . avy-goto-char-timer)
     ("M-C-M-s-s" . avy-goto-word-or-subword-1)))
...
#+BEGIN_SRC emacs-lisp
  (global-set-key (kbd "C-M-s-o") 'delete-forward-char)
  (global-set-key (kbd "C-M-s-u") 'delete-backward-char)
...
#+BEGIN_SRC emacs-lisp
  (use-package multiple-cursors
    :ensure t
    :bind (("C-M-s-c C-M-s-c" . mc/edit-lines)
...
#+begin_src emacs-lisp
  (use-package corral
    :ensure t
    :bind (("C-M-s-9" . corral-parentheses-backward)
           ("C-M-s-0" . corral-parentheses-forward)
           ("C-M-s-[" . corral-brackets-backward)
           ("C-M-s-]" . corral-brackets-forward)
           ("C-M-s-{" . corral-braces-backward)
           ("C-M-s-}" . corral-braces-forward)
           ("C-M-s-\"" . corral-double-quotes-backward))
    :config
...
#+begin_src emacs-lisp
  (use-package langtool
    :ensure t
    :bind (("C-M-s-c l" . langtool-check)
           ("C-M-s-c d" . langtool-check-done))
    :config
...
#+begin_src emacs-lisp
  (use-package powerthesaurus
    :ensure t
    :bind ("C-M-s-Z" . powerthesaurus-lookup-word-dwim))
...
Requires a move from package.el to straight.el, or manual installation. 
...
TODO: With Emacs 29, Eglot is part of the emacs core, so consider a switch.

#+begin_src emacs-lisp
  (use-package lsp-mode
    :ensure t
    :init
    (setq lsp-keymap-prefix "C-M-s-SPC")
    :hook ((js2-mode . lsp)
           (LaTeX-mode . lsp))
    :commands lsp
    :config
    ...
    (add-hook 'lsp-ui-imenu-mode-hook (lambda ()
                                        (visual-line-mode 0)))
    (setq lsp-ui-enable-snippet nil)
...
Additionally, use the LSP server ~ts-ls~, which LSP can automagically install.
#+begin_src emacs-lisp
...
**** js2-refactor
Not entirely sure how I've lived without this. Some of my particular facourites
are
| Name                | Keybinding | Description                                         |
|---------------------+------------+-----------------------------------------------------|
| log-this            | lt         | Add log statement for what's at point               |
| extract-let         | el         | takes makred expression and replace with a variable |
| rename-var          | rv         | Renames var i lexical scope                         |
| introduce-parameter | ip         | Changes expression to a parameter                   |

#+BEGIN_SRC emacs-lisp
  (use-package js2-refactor
    :ensure t
    :hook (js2-mode . js2-refactor-mode)
    :config (js2r-add-keybindings-with-prefix "C-M-s-r"))
#+END_SRC
...
*** MATLAB
Who would have thought that I'd end up using matlab again...
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package tex
    ...
    ;; 	("C-M-s-<tab>" . indent-for-tab-command)
    ;; 	("M-q" . sse/fill-paragraph-sentences))
    :config
    )

  ;; For some reason binding these keys doesn't work with :bind
...
** lsp
We use ~texlab~ as out language server.
...
<2023-04-25 Tue> Disabling this. Using my custom wrap-region function makes this annoying.
...
** COMMENT LaTeX table wizard
Bring some of the magic from org's table handling to Emacs
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp
    ...
    ;; Disabled; I also want pdf tools on macos
...
** org-screenshot
Allows us to attach a screenshot to an org-buffer. User a prefix to take a screenshot of the emacs windows it is called from.
#+BEGIN_SRC emacs-lisp
...
* Email mu4e
Currently using mu4e, only set up with Protonmail.

- Dependencies are mbsync, mu, and protonmail bridge.
- Some resources:
  - https://www.reddit.com/r/emacs/comments/bfsck6/mu4e_for_dummies/
- OBS: Note that org-msg need a newer version of mu4e (And hence mu) than the
  default shipped with ubuntu 20.04. One solution to this is to compile from
  source. load-path is set according to this below.
...
#+begin_src emacs-lisp
  (use-package mu4e
    ...
    (setq mu4e-get-mail-command "mbsync -a"
          ...
          mu4e-update-interval 240)             ; update every 4 minutes
    ...
    ;; Set user agent. Needed for org-msg
    ...
    ;; CUSTOMIZATION
    ;; A deletion is just moving a mil to the Trash folder, i.e. we do not set -T
    ...
    (define-key mu4e-headers-mode-map (kbd "L") 'org-store-link)
    (define-key mu4e-view-mode-map    (kbd "L") 'org-store-link)

    (add-hook 'mu4e-headers-mode-hook (lambda () (visual-line-mode 0)))
    (setq mu4e-search-threads nil)

    ;; bookmarks, mostly just hiding drafts
    (setq mu4e-bookmarks
          '(( :name  "Unread messages"
                     :query "flag:unread AND NOT flag:trashed"
                     :key ?u)
            ( :name "Today's messages"
                    :query "date:today..now AND NOT maildir:/sdu/Drafts AND NOT maildir:/proton/Drafts"
                    :key ?t)
            ( :name "Last 7 days"
                    :query "date:7d..now AND NOT maildir:/sdu/Drafts AND NOT maildir:/proton/Drafts"
                    :hide-unread t
                    :key ?w)
    ...
    ;; Use vertical view-mode. Optimal when I use a designated emacs instance for email
    ...
    ;; CONTEXTS
    ;; Set context. Currently sending emails only work for SDU
    (setq mu4e-context-policy 'pick-first)
    (setq mu4e-compose-context-policy 'ask-if-none)
    (setq mu4e-contexts
          (list
           (make-mu4e-context
            :name "sdu" ;;for SDU email
            :enter-func (lambda () (mu4e-message "Entering context sdu"))
            :leave-func (lambda () (mu4e-message "Leaving context sdu"))
            :match-func (lambda (msg)
                          (when msg
                            (string-prefix-p "/sdu" (mu4e-message-field msg :maildir))
                            ;; (mu4e-message-contact-field-matches
                            ;;  msg '(:from :to :cc :bcc) "erfurth@imada.sdu.dk")
                            ))
            :vars '((user-mail-address . "erfurth@imada.sdu.dk")
                    (user-full-name . "Simon Erfurth")
                    (mu4e-sent-folder . "/sdu/Sent")
                    (mu4e-drafts-folder . "/sdu/Drafts")
                    (mu4e-trash-folder . "/sdu/Trash")
                    (org-msg-signature . "
  ...
  University of Southern Denmark
   ...
   ,#+end_signature")
                    (mu4e-maildir-shortcuts . ( ("/sdu/Inbox" . ?i)
                                                ("/sdu/Events" . ?e)
                                                ("/sdu/Archive" . ?a)
                                                ("/sdu/Drafts" . ?D)
                                                ("/sdu/Sent" . ?s)
           ...
           (make-mu4e-context
            :name "proton" 
            :enter-func (lambda () (mu4e-message "Entering context proton"))
            :leave-func (lambda () (mu4e-message "Leaving context proton"))
            :match-func (lambda (msg)
                          (when msg
                            (string-prefix-p "/proton" (mu4e-message-field msg :maildir))
                            ;; (mu4e-message-contact-field-matches
                            ;;  msg '(:from :to :cc :bcc) "simon@serfurth.dk")
                            ))
            :vars '((user-mail-address . "simon@serfurth.dk")
                    (user-full-name . "Simon Erfurth")
...
Notes: 
mbsync -a (this is called automatically from mu4e)
mu init --maildir=~/.mail --my-address=erfurth@imada.sdu.dk --my-address=simon@serfurth.dk
mu index


** COMMENT thread folding
Disabled for now; interferes with mu4e-column-faces (and with toggle show
related (=W=) is not strictly necessary.)
...
** return to open attachment in emacs
I like to press return on attachments, and have them open in emacs.
#+begin_src emacs-lisp
...
** Composing emails: org-msg
Fantastic, and allows attaching attachments similar to org-attach.
...
** mu4e-org-link
Stolen from mu4easy https://github.com/danielfleischer/mu4easy/blob/master/mu4easy.el

Ideal for referring to mails in org mode, this simply gives a nicer
link/description.
...
** mu4e-alert
Essential to get desktop notifications.
...
** mu4e-goodies
Stolen from https://github.com/panjie/mu4e-goodies, don't want all their functions

This is not optimal, but it renames to current buffer, so we can visit it afterwards. 
#+begin_src emacs-lisp
...
** mu4e-overview
Source file manually changed to work - hence do not =:ensure t=.

#+begin_src emacs-lisp
  (use-package mu4e-overview
    :after mu4e
    :config
    (define-key mu4e-main-mode-map "o" 'mu4e-overview))
#+end_src
...
#+begin_src emacs-lisp
  ...
  _F_: forward | _C-+_: show more   | _A_: mk4actn | _H_: help      | _;_: context-switch
    ...
    ;; Bind keys as defined above
    (define-key mu4e-headers-mode-map (kbd "{") 'mu4e-headers-query-next)
    ...
    (define-key mu4e-headers-mode-map (kbd ".") 'hydra-mu4e-headers/body)
    )
#+end_src
...
You should also make sure that ~emacs --daemon~ runs on startup.
...
* emacs on macos
- Use https://hyperkey.app/ to remap capslock to C-M-s-. Then change all
  bindings that are either H- or C-M-s- to C-M-s- prefix. If this doesn't
  work, karabiner elements can do the same (and more) but isa heavier program.
- Unbind s-[spc] from changing input source in setting.
