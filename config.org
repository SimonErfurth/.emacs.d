...
#+begin_src emacs-lisp
  (set-face-attribute 'default nil :font "JetBrains Mono" :height 140 :weight 'light)
#+end_src
...
"New minor mode 'pixel-scroll-precision-mode'. When enabled, and if your mouse
supports it, you can scroll the display up or down at pixel resolution,
according to what your mouse wheel reports. Unlike 'pixel-scroll-mode', this
mode scrolls the display pixel-by-pixel, as opposed to only animating
...
** no backup files
remove all the ~ files all over the place.
#+begin_src emacs-lisp
...
** macOS emoji
Activates the macOS emoji picker.
#+begin_src emacs-lisp
  (global-set-key (kbd "C-M-s-e") 'ns-do-show-character-palette)
#+end_src
...
Do note that ltex-ls is not under development, so rather one should use
ltex-ls-plus, which can be found here https://github.com/ltex-plus/ltex-ls-plus
When using this, one need to modify the binary `ltex-ls-plus` to be named
`ltex-ls` in the path.
...
#+begin_src emacs-lisp
  (use-package eglot-ltex
    ...
    :init
    (setq eglot-ltex-server-path "/Users/erfurth/ltex-ls-plus-18.5.1/"
...
* python
Simple python mode, relying on eglot to do the heavy lifting. Currently using
pyright as language server, and black as code formatter.

`brew install pyright black`.
#+begin_src emacs-lisp
  (use-package python
...
* markdown
Mardown mode is used for gptel.
...
* llm's
gptel seems to be the right choice. Many packages add functionality to this.
